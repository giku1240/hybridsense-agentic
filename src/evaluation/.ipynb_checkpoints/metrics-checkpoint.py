...
class HybridSenseEvaluator:
    ...
    """
    def __init__(self, harness, text_data_path, physio_data_path, ablation_mode="full"):
    ...
    def evaluate_harness_routing(self, save_dir="data/evaluation"):
        """
        Tests the Agentic Harness against the real hold-out text and real high-risk vitals.
        Saves a detailed JSON report to save_dir/routing_report_{ablation_mode}_{timestamp}.json
        """
        ...
        for route, data in metrics.items():
            ...
            breakdown[route] = {
                ...
                "accuracy_pct": round((data["true_positive"] / expected) * 100, 2) if expected > 0 else None
            }

        print("\n--- Harness Routing Accuracy Report (Real Data) ---")
        print(f"Overall Accuracy: {overall_accuracy:.2f}% ({correct_routes}/{total_tests})")
        print("\nBreakdown by Route:")
        for route, data in breakdown.items():
            if data["total_expected"] > 0:
                print(f"  {route}: {data['accuracy_pct']}% ({data['true_positive']}/{data['total_expected']})")

        # --- Persist results to JSON ---
        os.makedirs(save_dir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join(save_dir, f"routing_report_{self.ablation_mode}_{timestamp}.json")
