...
def run_pipeline(config_path, resume=False, ablation_mode=None):
    """
    Args:
        ...
        ablation_mode: Override the ablation_mode in the config file.
                       One of: 'full', 'no_hrv', 'text_only'. If None, uses config value.
    """
    ...
    # CLI --ablation overrides the value in pipeline_config.yaml
    if ablation_mode is not None:
        logger.info(f"Ablation mode overridden by CLI: {ablation_mode}")
        dora_config['ablation_mode'] = ablation_mode
    effective_ablation = dora_config.get('ablation_mode', 'full')
    logger.info(f"Effective ablation mode: {effective_ablation}")
    ...
    if os.path.exists(sft_dataset_path):
        try:
            model_path = run_dora_training(
                ...
                ablation_mode=effective_ablation,
    ...
    # Skip if a trained PPO policy already exists
    ppo_output = config.get('ppo_training', {}).get('output_path', 'models/rl/ppo_jitai_policy')
    ppo_artifact = ppo_output if ppo_output.endswith('.zip') else ppo_output + '.zip'
    if os.path.exists(ppo_artifact):
        logger.info(f"PPO policy already exists at {ppo_artifact}. Skipping Stage 2.")
    else:
        try:
            run_ppo_training()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["full", "no_hrv", "text_only"],
        help=(
            "Override the ablation_mode in pipeline_config.yaml. "
            "One of: full, no_hrv, text_only. "
    ...
    run_pipeline(args.config, resume=resume_value, ablation_mode=args.ablation)
